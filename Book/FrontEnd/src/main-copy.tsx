...
export type HlCardProps = {
  props: {
    hotel: Hotel;
  };
};
export type HlGridProps = {
  props: {
    searchString: string;
  };
};
...
// UTILS
const createUtils = () => {
  const debouncefn = <T extends (...args: any[]) => void>(fn: T, delay = 500) => {
...
export const utilsFactory = createUtils();
// ------------------------------------------------------------
// SERVICES
const createServices = () => {
  const { HOTELS } = constantFactory;
...
const createComponents = () => {
  const { getHotels } = serviceFactory;
  const { debouncefn } = utilsFactory;
  const HlCard = ({ props }: HlCardProps) => {
    const { hotel } = props;
    return (
  ...
  const HlGrid = ({ props }: HlGridProps) => {
    const { searchString } = props;
    const { data = [], isLoading, isError } = useQuery({ queryKey: ['Hotels'], queryFn: getHotels });
    const filterHotels = data.filter((hotel) => hotel.name.toLowerCase().includes(searchString.toLowerCase()) || hotel.name.toLowerCase().includes(searchString.toLowerCase()));
    return (
      <>
          ...
          <div className="empty-state">
            {!isLoading && filterHotels.length < 1 && (
              <>
                ...
                <p>
                  Try searching with a different keyword, then "<strong>{searchString}</strong>".
                </p>
              </>
            )}
            {isLoading && `Loading...`}
  ...
  const HlSearch = () => {
    ...
    return (
      <section className="hl-search">
        ...
        />
        <HlGrid props={{ searchString: search }} />
      </section>
    );
  };
...
export const componentFactory = createComponents();

const { HlApp } = componentFactory;
